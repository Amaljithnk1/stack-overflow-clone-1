...
const PublicSpace = () => {
  ...
  const fetchPosts = async () => {
    try {
      const response = await axios.get('http://localhost:5000/public-posts');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:5000/public-posts', formData, {
        headers: {
  ...
  const handleDelete = async (postId) => {
    const token = user.token;
    try {
      await axios.delete(`http://localhost:5000/public-posts/${postId}`, {
        headers: {
          'Authorization': `Bearer ${token}`,
        },
      });
      ...
      <div className="public-space-posts">
        {posts.map((post) => (
          <div key={post._id} className="public-space-post">
            <div className="post-header">
              <div className="post-user-info">
                {post.creator && post.creator.avatar && (
                  <img src={`http://localhost:5000/${post.creator.avatar}`} alt="Avatar" className="post-avatar" />
                )}
            ...
            {post.videoUrl && (
              <CustomVideoPlayer videoSrc={`http://localhost:5000/${post.videoUrl}`} /> 
            )}
